...
function getClassEntry(obj) {
  ...
  for (let elem of obj.body) {
    if (elem.type === "ClassDeclaration") {
      ...
      elem.type === "ExportDefaultDeclaration" &&
      elem.declaration.type === "ClassDeclaration"
    ) {
...
function grabAttr(arrOfAttr) {
  return arrOfAttr.reduce((acc, curr) => {
    if (curr.value.type === "JSXExpressionContainer") {
      if (
        curr.value.expression.type === "ArrowFunctionExpression" ||
        curr.value.expression.type === "FunctionExpression"
      ) {
        if (curr.value.expression.body.body) {
          acc[curr.name.name] =
            curr.value.expression.body.body[0].expression.callee.name;
        } else {
        ...
        acc[curr.name.name + "True"] = {
          condition: condition,
          value: consequent,
        };

        acc[curr.name.name + "False"] = {
          condition: condition,
          value: alternate,
        };
      } else {
...
const constructComponentProps = (returnObj) => {
  const output = {};
  output[returnObj.openingElement.name.name] = grabAttr(
    returnObj.openingElement.attributes
  );
  return output;
};
/**
...
function constructComponentTree(filePath, rootPath) {
  ...
  if (result && Object.keys(result.childProps).length > 0) {
    for (let childProp of result.childProps) {
      ...
      if (childPathSplit.length === 1) newFileName += ".js";
...
module.exports = {
  ...
  grabImportNameAndPath,
  grabAttr,
  digStateInBlockStatement,
  digStateInClassBody,
  grabState,
  getClassEntry,
};
