...
const Directory = ({
  ...
  let item = (
    <div
      className="list-item"
      onClick={(event) =>
        clickHandler(id, directory.path, directory.type, event)
      }
      onContextMenu={(event) =>
        openCreateMenu(id, directory.path, directory.type, event)
      }
    >
      <span className="octi-file-directory">{directory.name}</span>
