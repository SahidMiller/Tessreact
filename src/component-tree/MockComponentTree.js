...
class MockComponentTree extends PureComponent {
  ...
  renderStateProps(stateProps) {
    if (stateProps && Object.keys(stateProps).length) {
      ...
      Object.keys(stateProps).forEach((key) => {
        let value =
          typeof stateProps[key] === "Object"
            ? stateProps[key]
            : JSON.stringify(stateProps[key]);
        renderArr.push(
          <li>
            {key} :{value}
          </li>
      ...
      return <ul className="state_props">{renderArr}</ul>;
    }
  }
  ...
  renderChildProps(childProps) {
    ...
    if (childProps && childProps.length > 0) {
      ...
      return <ul className="comp_refs">{renderArr}</ul>;
    }
  }
  ...
  renderTree(treeObj) {
    ...
    return (
      <li key={"ct_node-li" + name} className="tree_row">
        <input
          type="checkbox"
          id={"ct_node-npt_" + name}
          key={"ct_node-npt_" + name}
        />

        <label
          key={"ct_node-lbl_" + name}
          id={"ct_node-lbl_" + name}
          className="tree_node"
          htmlFor={"ct_node-npt_" + name}
        >
          {name}
        </label>
            ...
            <span className="props-form">
              {Object.keys(stateProps).length && (
                <Fragment>
                  ...
                  <label htmlFor={"ct_state-npt_" + name}>
                    [state_props] ({Object.keys(stateProps).length})
                  </label>
                  <br />
              ...
              {childProps.length > 0 && (
                <Fragment>
                  ...
                  <label htmlFor={"ct_child-npt_" + name}>
                    [comp_props] ({childProps.length})
                  </label>
                  <br />
  ...
  render() {
    ...
    return (
      <ul className="tree">{this.renderTree(this.props.componentTreeObj)}</ul>
    );
  }
}
